...
def prepare_alpha_results_for_upsert(bootstrap_results: pd.DataFrame,
                                   skill_metrics: pd.DataFrame,
                                   fund_classifications: pd.DataFrame) -> List[Dict]:
    """
    ...
    try:
        ...
        merged_df = pd.merge(
            bootstrap_results,
            skill_metrics[['scheme_code', 'sharpe_ratio', 'info_ratio', 'composite_skill_score', 'percentile_rank']],
            on='scheme_code',
            how='left'
        )
        ...
        for fund in merged_df['scheme_code'].unique():
            ...
            sharpe = fund_data['sharpe_ratio'].iloc[0] if not fund_data['sharpe_ratio'].isnull().all() else 0
            info_ratio = fund_data['info_ratio'].iloc[0] if not fund_data['info_ratio'].isnull().all() else 0
            skill_score = fund_data['composite_skill_score'].iloc[0] if not fund_data['composite_skill_score'].isnull().all() else 0
            percentile_rank = fund_data['percentile_rank'].iloc[0] if not fund_data['percentile_rank'].isnull().all() else 0
            verdict = fund_data['overall_verdict'].iloc[0] if not fund_data['overall_verdict'].isnull().all() else 'Unknown'
