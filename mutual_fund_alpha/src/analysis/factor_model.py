...
def run_factor_regression(
    excess_returns: pd.Series, factors: pd.DataFrame, window: int = 756
) -> FactorResult:
    """
...
def run_rolling_regressions(
    ...
    try:
        ...
        for fund in funds:
            ...
            for i in range(0, len(fund_data) - window + 1, step):
                ...
                if len(factor_window) >= 10:  # Minimum observations
                    # Run regression
                    excess_return_series = window_data.set_index("date")[
                        "excess_return"
                    ]
                    factor_data = factor_window.set_index("date")[
                        ["mkt_rf", "smb", "hml"]
                    ]

                    try:
                        reg_result = run_factor_regression(
                            excess_return_series, factor_data, window
                        )
        ...
        logger.info(
            f"Completed {len(results)} rolling regressions for {len(funds)} funds"
        )
        return results
