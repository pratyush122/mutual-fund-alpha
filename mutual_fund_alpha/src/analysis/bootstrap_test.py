...
def bootstrap_alpha_test(
    residuals: np.ndarray, observed_alpha: float, n_bootstrap: int = 1000
) -> Dict:
    """
    ...
    try:
        ...
        p_value = (
            sum(1 for alpha in bootstrap_alphas if alpha >= observed_alpha)
            / n_bootstrap
        )
...
def perform_bootstrap_tests(
    ...
    """
    logger.info(
        f"Performing bootstrap tests for {len(regression_results)} regressions..."
    )
...
def classify_funds_by_skill(bootstrap_results: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        for fund in bootstrap_results["scheme_code"].unique():
            ...
            probably_skilled_count = sum(
                fund_data["bootstrap_verdict"] == "Probably Skilled"
            )
            ...
            fund_classifications.append(
                {
                    ...
                    "skill_percentage": (
                        (skilled_count + probably_skilled_count) / total_windows * 100
                        if total_windows > 0
                        else 0
                    ),
