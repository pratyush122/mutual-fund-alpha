...
def create_tables() -> None:
    ...
    try:
        ...
        logger.info(
            "1. funds table with columns: scheme_code (TEXT, PK), scheme_name (TEXT), category (TEXT), aum_cr (REAL), inception_date (DATE)"
        )
        logger.info(
            "2. nav_history table with columns: id (INTEGER, PK), scheme_code (TEXT, FK), date (DATE), nav (REAL), daily_return (REAL)"
        )
        logger.info(
            "3. factor_data table with columns: date (DATE, PK), mkt_rf (REAL), smb (REAL), hml (REAL), rf (REAL)"
        )
        logger.info(
            "4. alpha_results table with columns: id (INTEGER, PK), scheme_code (TEXT, FK), computed_at (TIMESTAMP), window_days (INTEGER), alpha (REAL), beta_mkt (REAL), beta_smb (REAL), beta_hml (REAL), r_squared (REAL), t_stat_alpha (REAL), p_value (REAL), sharpe (REAL), info_ratio (REAL), skill_score (REAL), percentile_rank (REAL), alpha_persistence (REAL), factor_exposures (JSON), rolling_windows (JSON)"
        )
        ...
        logger.info(
            "Please create the tables in Supabase dashboard first, then run the seeding script."
        )
