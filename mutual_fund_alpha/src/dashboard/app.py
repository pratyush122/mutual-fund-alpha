...
def fund_screener_page(data: Dict[str, pd.DataFrame]) -> None:
    ...
    if 'fund_classifications' in data:
        df = pd.merge(df, data['fund_classifications'][['scheme_code', 'overall_verdict']],
    ...
    selected_verdict = st.sidebar.selectbox("Skill Classification", verdict_options,
...
def fund_xray_page(data: Dict[str, pd.DataFrame]) -> None:
    ...
    # Get fund data
    fund_skill = data['skill_metrics'][data['skill_metrics']['scheme_code'] == selected_fund].iloc[0]
    fund_regression = data['regression_results'][data['regression_results']['scheme_code'] == selected_fund]
    ...
    if not fund_regression.empty:
        fig_alpha = px.line(fund_regression, x='start_date', y='alpha',
        ...
        fig_factors = px.bar(factor_data, x='Factor', y='Exposure',
                            title=f"Average Factor Exposures for {selected_fund}",
                            color='Exposure', color_continuous_scale='RdBu')
...
def category_benchmarking_page(data: Dict[str, pd.DataFrame]) -> None:
    ...
    if 'fund_classifications' in data:
        ...
        fig_box = px.box(df, x='overall_verdict', y='composite_skill_score',
                        title="Skill Score Distribution by Classification",
                        labels={'overall_verdict': 'Classification', 'composite_skill_score': 'Skill Score'})
        st.plotly_chart(fig_box, use_container_width=True)

    # Scatter plot: Skill Score vs Consistency
    st.subheader("Skill vs Consistency")

    fig_scatter = px.scatter(df, x='consistency_percentage', y='composite_skill_score',
                            hover_data=['scheme_code'],
                            title="Skill Score vs Consistency Percentage",
                            labels={'consistency_percentage': 'Consistency %', 'composite_skill_score': 'Skill Score'})
    ...
    if 'overall_verdict' in df.columns:
        category_avg = df.groupby('overall_verdict').agg({
...
def data_health_page(data: Dict[str, pd.DataFrame]) -> None:
    ...
    if os.path.exists(validation_file):
        ...
        for dataset_name, dataset_info in validation_report.get('validations', {}).items():
            ...
            if 'error' in dataset_info:
                ...
                st.write(f"- Date range: {dataset_info.get('date_range', {}).get('start', 'N/A')} to {dataset_info.get('date_range', {}).get('end', 'N/A')}")
